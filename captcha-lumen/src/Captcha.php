...
class Captcha
{
    ...
    {
        $this->captchaConf = config('Captcha\Yangbx');
        ...
        $this->hasher = $hasher;
        $this->str = $str;
        $this->characters = isset($this->captchaConf['captchaCharacters']) ? $this->captchaConf['captchaCharacters'] : '2346789abcdefghjmnpqrtuxyzABCDEFGHJMNPQRTUXYZ';
    }
    ...
    {
        foreach ($this->captchaConf as $key => $val) {
            $this->{$key} = $val;
        }
    }
    /**
    ...
    protected function generateById($captchaId)
    {
        ...
        $cacheTime = isset($this->captchaConf['usefulTime']) ? $this->captchaConf['usefulTime'] : 5;
     ...
     * @return mixed
     */
    public function createById($captchaId)
    {
     ...
     * @return bool
     */
    public function checkCaptchaById($captchaCode, $captchaId)
    {
        $captcha = 'captcha_'.$captchaId;
        if (! Cache::has($captcha)) {
            return false;
        }
        $captchaValue = Cache::get($captcha);
        if (! $this->sensitive) {
            $captchaValue = strtolower($captchaValue);
